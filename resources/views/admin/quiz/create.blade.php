<x-app-layout>
    <x-slot:header>Quiz Oluştur</x-slot>
    <div class="p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div class="overflow-x-auto relative sm:rounded-lg mb-5">
            <form action="{{ route('quizzes.store') }}" method="POST">
                ...
                <div class="mb-6 px-2 mt-4">
                    <input id="isFinished" type="checkbox" name="isFinished" @if (old('isFinished') == "on") checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
                    <label for="isFinished" class="ml-2 text-sm font-medium text-gray-900">Bitiş tarihi olacak mı?</label>
                </div>
                <div class="mb-6 px-2 mt-4 @if (old('isFinished') != "on") hidden @endif" id="finishTime">
                    <label for="finished_at" class="block mb-2 text-sm font-medium text-gray-900">Bitiş Tarihi</label>
                    <input type="datetime-local" id="finished_at" name="finished_at" value="{{ old('finished_at') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                </div>
                <div class="mb-6 px-2">
